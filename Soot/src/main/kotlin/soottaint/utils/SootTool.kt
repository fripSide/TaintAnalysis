...
//        println(Scene.v().entryPoints)
	...
	fun initSootAndroid(ajar_path: String, apk_dir: String) {
		Options.v().set_src_prec(Options.src_prec_apk);
		Options.v().set_process_dir(Collections.singletonList(apk_dir));
		Options.v().set_force_android_jar(ajar_path);
...
//        PackManager.v().runPacks()
	...
	fun initMySoot(load_path: String) {
		soot.G.reset()
		Options.v().set_src_prec(Options.src_prec_class)
		Options.v().set_process_dir(arrayListOf(load_path))
		Options.v().set_keep_line_number(true)
...
//		PackManager.v().writeOutput()
	...
	fun filterClasses(prefix: String): List<SootClass> {
		val clsList = arrayListOf<SootClass>()
	...
	fun dumpMethod(mtd: SootMethod) {
		var body = tryGetMethodBody(mtd) ?: return
		var bg = CompleteBlockGraph(body)
		println(bg)
		println(body.defBoxes)
		println(body.useBoxes)
		println(body.useAndDefBoxes)
		body?.units?.forEach { unix ->
			if (unix is Stmt) {
				println("stmt:" + unix.javaClass)
			}
			if (unix is JAssignStmt) {
				println(unix.leftOp.type)
				println("type: " + unix.rightOp.type)
				var sc = Scene.v().getSootClass(unix.rightOp.type.toString())
//							println("SootClass: " + sc)
//							for (mtx in sc.methods) {
//								println(mtx)
//							}
			} else if (unix is InvokeStmt) {
				println(unix)
			}
